...
def get_pigs():
    ...
    pigl = []
    for pig in pigs:
        pigl.append(str(pig.h2.string))
    jsonlist = json.dumps(pigl)
    return str(jsonlist)
...
def get_story_headings():
    soup = BeautifulSoup(html_doc)
    stl = []
    for elem in soup(text=re.compile(r'Story')):
        stl.append(str(elem))
    jsonlist = json.dumps(stl)
    return str(jsonlist)
...
def get_houses():
    ...
    for row in soup.find_all('tr'):
        temp1 = []
        aux = row.find_all('td')
        temp1.append(str(aux[0].string))
        temp1.append(str(aux[1].string))
...
def treasure_hunting():
    html1 = urllib2.urlopen('http://www.example.com/')
    soup1 = BeautifulSoup(html1)
    link = soup1.find('a').get('href')
    html2 = urllib2.urlopen(link)
    soup2 = BeautifulSoup(html2)
    rows = soup2.find_all('tr')
    temp = rows[3].find('td').text
    return unicode(temp)
