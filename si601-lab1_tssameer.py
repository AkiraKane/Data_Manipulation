...
def bananas(count):
    if count == 0:
        ...
        for i in range(count):
            banlp = banlp + str(i+1) + ' banana, '
        return banlp[:len(banlp)-2]
    else:
        banlp = '1 banana, 2 banana, 3 banana, 4 banana, 5 banana and ' + str(count-5) + ' more bananas'
        return banlp
...
def match_ends(words, starting, ending):
    ...
    enders=[]
    for word in words:
        if word.startswith(starting) and len(word) >6:
            starters.append(word)
        if word.endswith(ending) and len(word) >6:
            enders.append(word)
    s1 = set(starters)
    s2 = set(enders)
    matchers = s1.intersection(s2)
    return len(matchers)
...
def unique_counts(words):
    countd = dict()
    for word in words:
        countd[word] = words.count(word)
    counts = sorted(countd.items(), key= lambda x: (-x[1],x[0]))
    return counts
